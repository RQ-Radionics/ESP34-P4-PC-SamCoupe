/*
 * sim_kbd.c — USB HID keyboard driver for SimCoupe (Olimex ESP32-P4-PC)
 *
 ...
 *   - Scancodes delivered byte-by-byte via FreeRTOS queue
 *
 * PS/2 Set 2 encoding:
 *   Normal make:    <make>
...
static QueueHandle_t          s_scancode_queue = NULL;
...
/* --------------------------------------------------------------------------
 * Scancode emission — enqueue bytes to FreeRTOS queue
 * -------------------------------------------------------------------------- */

static inline void emit_byte(uint8_t b)
{
    if (s_scancode_queue) {
        xQueueSendToBack(s_scancode_queue, &b, 0);
    }
}
...
{
    if (!scancode_queue) return ESP_ERR_INVALID_ARG;
    s_scancode_queue = scancode_queue;
    s_running = true;
    ...
    gpio_config_t io_conf = {
        .pin_bit_mask = (1ULL << CONFIG_SIM_KBD_HUB_RST_GPIO),
    ...
    gpio_config(&io_conf);
    ESP_LOGI(TAG, "Hub reset: asserting GPIO%d LOW", CONFIG_SIM_KBD_HUB_RST_GPIO);
    gpio_set_level(CONFIG_SIM_KBD_HUB_RST_GPIO, 0);
    ...
    if (ret != ESP_OK) {
        ...
        gpio_set_level(CONFIG_SIM_KBD_HUB_RST_GPIO, 1);
        return ret;
    }
     ...
     * SPLIT transactions. Must be set before hub reset is released. */
#if CONFIG_IDF_TARGET_ESP32P4
