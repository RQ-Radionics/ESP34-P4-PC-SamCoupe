...
void Breakpoint::Add(Breakpoint&& bp)
{
    ...
    if ((bp.type == BreakType::Until || bp.type == BreakType::Temp) && bp.expr)
    {
        if (auto val = bp.expr.TokenValue(Expr::TokenType::Number))
        {
            ...
            {
                char buf[32];
                snprintf(buf, sizeof(buf), "PC==%x", *value);
                bp.expr = Expr::Compile(buf);
            }
        }
    }
...
std::string to_string(const Breakpoint& bp)
{
    ...
    case BreakType::Until:
        result = fmt::format("UNTIL {}", bp.expr.str);
        break;
    ...
    case BreakType::Execute:
        if (auto exec = std::get_if<BreakExec>(&bp.data))
        {
            ...
            result = fmt::format("EXEC {}:{:04X}", page_desc, page_offset);
        }
        break;
    ...
    case BreakType::Memory:
        if (auto mem = std::get_if<BreakMem>(&bp.data))
        {
            ...
            auto page_desc = Memory::PageDesc(PtrPage(pPhysAddr), true);
            auto page_offset = PtrOffset(pPhysAddr);
            result = fmt::format("MEM {}:{:04X}", page_desc, page_offset);

            if (mem->phys_addr_to != pPhysAddr)
            {
                extent = static_cast<int>((uint8_t*)mem->phys_addr_to - (uint8_t*)pPhysAddr);
                result += fmt::format(" L{:04X}", (extent + 1));
            }

            result += fmt::format(" {}", to_string(mem->access));
        }
        break;
    ...
    case BreakType::Port:
        if (auto port = std::get_if<BreakPort>(&bp.data))
        {
            if (port->compare <= 0xff)
                result = fmt::format("PORT {:02X} {}", port->compare, to_string(port->access));
            else
                result = fmt::format("PORT {:04X} {}", port->compare, to_string(port->access));
        }
        break;
    ...
    case BreakType::Interrupt:
        ...
        {
            result = "INT ";
            if (intr->mask & STATUS_INT_FRAME)  result += "FRAME ";
            if (intr->mask & STATUS_INT_LINE)   result += "LINE ";
            if (intr->mask & STATUS_INT_MIDIOUT) result += "MIDIOUT ";
            if (intr->mask & STATUS_INT_MIDIIN) result += "MIDIIN ";
        }
        break;
    ...
    if (pPhysAddr)
    {
        ...
        {
            if (extent)
                result += fmt::format(" (@{:04X}-{:04X},@{:04X}-{:04X})",
                    *addr2, (*addr2 + extent), *addr1, (*addr1 + extent));
            else
                result += fmt::format(" (@{:04X},@{:04X})", *addr2, *addr1);
        }
        else if (addr1.has_value())
        {
            if (extent)
                result += fmt::format(" (@{:04X}-{:04X})", *addr1, (*addr1 + extent));
            else
                result += fmt::format(" (@{:04X})", *addr1);
        }
    }

    if (bp.expr && bp.type != BreakType::Until)
        result += fmt::format(" if {}", bp.expr.str);
