...
// Single block holding all memory needed — allocated in PSRAM at runtime
...
// Contention tables — allocated in PSRAM at Init() time (119,872 bytes each)
uint8_t* contention_mode1 = nullptr;
uint8_t* contention_mode234 = nullptr;
uint8_t* contention_4T = nullptr;
const uint8_t* contention_ptr = nullptr;
...
bool Init(bool fFirstInit_/*=false*/)
{
    if (fFirstInit_)
    {
        // Allocate SAM RAM in PSRAM (4.5MB — too large for internal SRAM)
        if (!pMemory)
        {
            pMemory = static_cast<uint8_t*>(
                heap_caps_malloc(TOTAL_PAGES * MEM_PAGE_SIZE, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
            if (!pMemory)
        ...
        for (unsigned t = 0; t < CONTENTION_SIZE; ++t)
        {
