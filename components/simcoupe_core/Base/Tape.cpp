...
std::string GetBlockDetails(libspectrum_tape_block* block)
{
    ...
    if (!type.empty())
    {
        result = fmt::format("{}: '{}'", type, filename);
        if (!extra.empty())
            result += " " + extra;
    }
    else
    {
        ...
        case LIBSPECTRUM_TAPE_BLOCK_TURBO:
        {
            ...
            result = fmt::format("{} bytes", length);
            break;
        }

        case LIBSPECTRUM_TAPE_BLOCK_PURE_DATA:
        case LIBSPECTRUM_TAPE_BLOCK_RAW_DATA:
            result = fmt::format("{} bytes", libspectrum_tape_block_data_length(block));
            break;

        case LIBSPECTRUM_TAPE_BLOCK_PURE_TONE:
            result = fmt::format("{} tstates", libspectrum_tape_block_pulse_length(block));
            break;

        case LIBSPECTRUM_TAPE_BLOCK_PULSES:
            result = fmt::format("{} pulses", libspectrum_tape_block_count(block));
            break;

        case LIBSPECTRUM_TAPE_BLOCK_PAUSE:
            result = fmt::format("{}ms", libspectrum_tape_block_pause(block));
            break;
        ...
        case LIBSPECTRUM_TAPE_BLOCK_CUSTOM:
            result = libspectrum_tape_block_text(block);
            break;
        ...
        case LIBSPECTRUM_TAPE_BLOCK_JUMP:
        {
            ...
            if (offset >= 0)
                result = fmt::format("Forward {} blocks", offset);
            else
                result = fmt::format("Backward {} blocks", -offset);
            break;
        }

        case LIBSPECTRUM_TAPE_BLOCK_LOOP_START:
            result = fmt::format("{} iterations", libspectrum_tape_block_count(block));
            break;

        case LIBSPECTRUM_TAPE_BLOCK_SELECT:
            result = fmt::format("{} options", libspectrum_tape_block_count(block));
            break;

        case LIBSPECTRUM_TAPE_BLOCK_GENERALISED_DATA:
            result = fmt::format("{} data symbols",
        ...
        case LIBSPECTRUM_TAPE_BLOCK_HARDWARE:
        {
            ...
            for (size_t i = 0; i < count; i++)
            {
                ...
                if (id == 9)
                    ...
                    result += "Timex Sinclair";
                else
                    result += fmt::format("Unknown hardware ({:02x})", id);
            }
