...
// Instruction tracing — aTrace allocated in PSRAM at first use (84KB)
#define TRACE_SLOTS 1000
static TRACEDATA* aTrace = nullptr;
int nNumTraces;

static void ensure_trace_buf()
{
    if (!aTrace)
    {
        aTrace = static_cast<TRACEDATA*>(
            heap_caps_calloc(TRACE_SLOTS, sizeof(TRACEDATA), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
        if (!aTrace)
            esp_system_abort("Debug: failed to allocate trace buffer in PSRAM");
    }
}
    ...
    : Dialog(nullptr, 500, 260 + 36 + 2, "")
{
    ...
    if (bp_index.has_value())
    {
        ...
        if (bp.type == BreakType::Until)
            status_str = fmt::format("\aYUNTIL condition met:  {}", bp.expr.str);
        else if (bp.type != BreakType::Temp)
            status_str = fmt::format("\aYBreakpoint {} hit:  {}", *bp_index, to_string(bp));

        SetStatus(status_str, true, sPropFont);
