...
{ /* RST 00H */
    Z80Ops::addressOnBus(getPairIR().word, 1);
    if (Z80Ops::notifyRST && Z80Ops::notifyRST(0x00)) return;
    push(REG_PC);
    REG_PC = REG_WZ = 0x00;
}
...
{ /* RET Z */
    ...
    if ((sz5h3pnFlags & ZERO_MASK) != 0) {
        REG_PC = REG_WZ = pop();
        // notifyRETZ: Tape::RetZHook — return true to suppress the return
        if (Z80Ops::notifyRETZ && Z80Ops::notifyRETZ()) {
            // hook consumed the return (e.g. tape fast load); undo pop
            // by restoring PC — hook is responsible for setting PC correctly
        }
    }
...
{ /* RET */
    REG_PC = REG_WZ = pop();
    if (Z80Ops::notifyRET) Z80Ops::notifyRET();
}
...
{ /* RST 08H */
    Z80Ops::addressOnBus(getPairIR().word, 1);
    if (Z80Ops::notifyRST && Z80Ops::notifyRST(0x08)) return;
    push(REG_PC);
    REG_PC = REG_WZ = 0x08;
}
...
{ /* RST 10H */
    Z80Ops::addressOnBus(getPairIR().word, 1);
    if (Z80Ops::notifyRST && Z80Ops::notifyRST(0x10)) return;
    push(REG_PC);
    REG_PC = REG_WZ = 0x10;
}
...
{ /* RST 18H */
    Z80Ops::addressOnBus(getPairIR().word, 1);
    if (Z80Ops::notifyRST && Z80Ops::notifyRST(0x18)) return;
    push(REG_PC);
    REG_PC = REG_WZ = 0x18;
}
...
{
    Z80Ops::addressOnBus(getPairIR().word, 1);
    if (Z80Ops::notifyRST && Z80Ops::notifyRST(0x20)) return;
    push(REG_PC);
    REG_PC = REG_WZ = 0x20;
}
...
{
    Z80Ops::addressOnBus(getPairIR().word, 1);
    if (Z80Ops::notifyRST && Z80Ops::notifyRST(0x28)) return;
    push(REG_PC);
    REG_PC = REG_WZ = 0x28;
}
...
{
    Z80Ops::addressOnBus(getPairIR().word, 1);
    if (Z80Ops::notifyRST && Z80Ops::notifyRST(0x30)) return;
    push(REG_PC);
    REG_PC = REG_WZ = 0x30;
}
...
{
    if (Z80Ops::notifyEI) Z80Ops::notifyEI();
    ffIFF1 = ffIFF2 = true;
    pendingEI = true;
}
...
{
    Z80Ops::addressOnBus(getPairIR().word, 1);
    if (Z80Ops::notifyRST && Z80Ops::notifyRST(0x38)) return;
    push(REG_PC);
    REG_PC = REG_WZ = 0x38;
}
...
void Z80::decodeED(void) {
    switch (opCode) {
        ...
        { /* OUT (C),0 */
            ...
            Z80Ops::outPort(REG_BC, 0x00); // NMOS Z80 returns 0x00
            // Z80Ops::outPort(REG_BC, 0xFF); // CMOS Z80 returns 0xFF
...
///////////////////////////////////////////////////////////////////////////////
// Core execution functions (interrupt, NMI, checkINT, execute)
// Adapted from ESPectrum Z80_JLS.cpp — ESPectrum-specific calls removed.
///////////////////////////////////////////////////////////////////////////////
...
void Z80::interrupt(void) {
    ...
    if (modeINT == IntMode::IM2) {
        REG_PC = Z80Ops::peek16((regI << 8) | 0xff);
    } else {
        REG_PC = 0x0038;
    }
    REG_WZ = REG_PC;
...
void Z80::checkINT(void) {
    if (ffIFF1 && !pendingEI && Z80Ops::isActiveINT && Z80Ops::isActiveINT()) {
        lastFlagQ = false;
        interrupt();
    }
}
